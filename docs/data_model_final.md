...
Ce document décrit le schéma final des bases SQLite produites par les pipelines RNE et SIRENE. Les tables construites à partir du RNE proviennent des JSON RNE (stock et flux). Les tables SIRENE sont issues des fichiers CSV stock/flux diffusés par l'INSEE, parfois enrichies avec les données RNE (indicateurs `from_rne`/`date_mise_a_jour_rne` et insertions des SIREN manquants).
...
| RNE | JSON | `formality.content.formeExerciceActivitePrincipale` ou activité principale détectée | Ajout du code de forme d'exercice principale ; alimente aussi `nature_entreprise` dans l'`immatriculation`. | `forme_exercice_activite_principale` | TEXT |
...
| RNE | JSON | `etablissementPrincipal.adresse.{pays, codePays, commune, codePostal, codeInseeCommune, voie, numVoie, typeVoie, indiceRepetition, complementLocalisation, distributionSpeciale}` | Mapping champ à champ vers l'adresse du siège. | Adresse détaillée | TEXT |
...
## Tables SIRENE (sirene.db)

### Table `unite_legale`

| Data Source | Type source | Champ source | Transformation / Logique | Champ cible | Type cible |
| --- | --- | --- | --- | --- | --- |
| SIRENE (stock CSV) | CSV | `dateCreationUniteLegale` | Copie directe. | `date_creation_unite_legale` | DATE |
| SIRENE (stock CSV) | CSV | `sigleUniteLegale` | Copie directe. | `sigle` | TEXT |
| SIRENE (stock CSV) | CSV | `prenom1UniteLegale` | Copie directe pour la personne physique. | `prenom` | TEXT |
| SIRENE (stock CSV) | CSV | `identifiantAssociationUniteLegale` | Copie directe. | `identifiant_association_unite_legale` | TEXT |
| SIRENE (stock CSV) | CSV | `trancheEffectifsUniteLegale` | Copie directe. | `tranche_effectif_salarie_unite_legale` | TEXT |
| SIRENE (stock CSV) | CSV | `categorieEntreprise` | Copie directe. | `categorie_entreprise` | TEXT |
| SIRENE (stock CSV) | CSV | `etatAdministratifUniteLegale` | Copie directe. | `etat_administratif_unite_legale` | TEXT |
| SIRENE (stock CSV) | CSV | `nomUniteLegale` | Copie directe. | `nom` | TEXT |
| SIRENE (stock CSV) | CSV | `nomUsageUniteLegale` | Copie directe. | `nom_usage` | TEXT |
| SIRENE (stock CSV) | CSV | `denominationUniteLegale` | Copie directe. | `nom_raison_sociale` | TEXT |
| SIRENE (stock CSV) | CSV | `denominationUsuelle1UniteLegale` | Copie directe. | `denomination_usuelle_1` | TEXT |
| SIRENE (stock CSV) | CSV | `denominationUsuelle2UniteLegale` | Copie directe. | `denomination_usuelle_2` | TEXT |
| SIRENE (stock CSV) | CSV | `denominationUsuelle3UniteLegale` | Copie directe. | `denomination_usuelle_3` | TEXT |
| SIRENE (stock CSV) | CSV | `categorieJuridiqueUniteLegale` | Copie directe. | `nature_juridique_unite_legale` | TEXT |
| SIRENE (stock CSV) | CSV | `activitePrincipaleUniteLegale` | Copie directe. | `activite_principale_unite_legale` | TEXT |
| SIRENE (stock CSV) | CSV | `economieSocialeSolidaireUniteLegale` | Copie directe. | `economie_sociale_solidaire_unite_legale` | TEXT |
| SIRENE (stock CSV) | CSV | `statutDiffusionUniteLegale` | Copie directe. | `statut_diffusion_unite_legale` | TEXT |
| SIRENE (stock CSV) | CSV | `estSocieteMissionUniteLegale` | Copie directe. | `est_societe_mission` | TEXT |
| SIRENE (stock CSV) | CSV | `anneeCategorieEntreprise` | Copie directe. | `annee_categorie_entreprise` | TEXT |
| SIRENE (stock CSV) | CSV | `anneeTrancheEffectifsUniteLegale` | Copie directe. | `annee_tranche_effectif_salarie` | TEXT |
| SIRENE (stock CSV) | CSV | `caractereEmployeurUniteLegale` | Copie directe. | `caractere_employeur` | TEXT |
...
### Tables `etablissement`, `siege`, `flux_etablissement`

| Data Source | Type source | Champ source | Transformation / Logique | Champ cible | Type cible |
| --- | --- | --- | --- | --- | --- |
| SIRENE (stock CSV) | CSV | `siren`, `siret` | Copie directe des identifiants. | `siren`, `siret` | TEXT |
| SIRENE (stock CSV) | CSV | `dateCreationEtablissement` | Copie directe. | `date_creation` | DATE |
| SIRENE (stock CSV) | CSV | `trancheEffectifsEtablissement` | Copie directe. | `tranche_effectif_salarie` | TEXT |
| SIRENE (stock CSV) | CSV | `caractereEmployeurEtablissement` | Copie directe. | `caractere_employeur` | TEXT |
| SIRENE (stock CSV) | CSV | `anneeTrancheEffectifsEtablissement` | Copie directe. | `annee_tranche_effectif_salarie` | TEXT |
| SIRENE (stock CSV) | CSV | `activitePrincipaleRegistreMetiersEtablissement` | Copie directe. | `activite_principale_registre_metier` | TEXT |
| SIRENE (stock CSV) | CSV | `estSiege` | Copie directe. | `est_siege` | TEXT |
| SIRENE (stock CSV) | CSV | Champs adresse (`numeroVoieEtablissement`, `typeVoieEtablissement`, `libelleVoieEtablissement`, `codePostalEtablissement`, `libelleCommuneEtablissement`, `communeEtablissement`, `complementAdresseEtablissement`, `cedexEtablissement`, `libelleCedexEtablissement`, `distributionSpecialeEtablissement`, `indiceRepetitionEtablissement`) | Copie champ à champ pour l'adresse postale. | Colonnes adresse correspondantes | TEXT |
| SIRENE (stock CSV) | CSV | `dateDebut` | Copie directe. | `date_debut_activite` | DATE |
| SIRENE (stock CSV) | CSV | `etatAdministratifEtablissement` | Copie directe. | `etat_administratif_etablissement` | TEXT |
| SIRENE (stock CSV) | CSV | `enseigne{1,2,3}Etablissement` | Copie directe. | `enseigne_1`, `enseigne_2`, `enseigne_3` | TEXT |
| SIRENE (stock CSV) | CSV | `activitePrincipaleEtablissement` | Copie directe. | `activite_principale` | TEXT |
| SIRENE (stock CSV) | CSV | `nomCommercialEtablissement` | Copie directe. | `nom_commercial` | TEXT |
| SIRENE (stock CSV) | CSV | Champs étranger (`libelleCommuneEtrangerEtablissement`, `codePaysEtrangerEtablissement`, `libellePaysEtrangerEtablissement`) | Copie directe. | `libelle_commune_etranger`, `code_pays_etranger`, `libelle_pays_etranger` | TEXT |
...
| SIRENE (flux CSV) | CSV | Identiques aux colonnes ci-dessus | Chargement des flux pour réécriture/merge dans `etablissement` et `siege`. | Colonnes miroir | Types identiques |
...
### Tables `dirigeant_pp` et `dirigeant_pm`
